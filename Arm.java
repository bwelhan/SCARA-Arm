...
public class Arm
{
    ...
    public boolean inverseKinematic(double xt_new,double yt_new){
        ...
        double gamma1 = Math.atan2(dy1, dx1);
        double alpha1 = Math.PI - gamma1;

        // elbows positions
        double new_xj1 = xm1 + 0.5*(xt-xm1) + h1*Math.cos(Math.PI/2 - alpha1);
        double new_yj1 = ym1 + 0.5*(yt-ym1) + h1*Math.sin(Math.PI/2 - alpha1);

        theta1 = Math.atan2(new_yj1-ym1, new_xj1-xm1);
        ...
        double gamma2 = Math.atan2(dy2, dx2);
        double alpha2 = Math.PI - gamma2;

        // elbows positions
        double new_xj2 = xm2 + 0.5*(xt-xm2) - h2*Math.cos(Math.PI/2 - alpha2);
        double new_yj2 = ym2 + 0.5*(yt-ym2) - h2*Math.sin(Math.PI/2 - alpha2);

        double elbowMag = Math.sqrt((new_xj2-new_xj1)*(new_xj2-new_xj1) + (new_yj2-new_yj1)*(new_yj2-new_yj1));

        //------- Singularity Tests----- \\

        // Elbows too far away
        if( elbowMag > 0.95*2*r){
            UI.println("Singularity, elbows too far apart");
            valid_state = false;
            return false;
        }
        ...
        //Elbows too close
        if(elbowMag < 0.8*r){
            UI.println("Singularity, elbows too close");
            valid_state = false;
            return false;
        }
    ...
    public int get_pwm1(){
        ...
        pwm = (int)(-9.8744 * (theta1*180/Math.PI) + 224.95);
        //pwm = (int)(224.96 + (-theta2 - 90)*9.8744);  // Given to us
        return pwm;
    }
    ...
    public int get_pwm2(){
        ...
        pwm = (int)(-9.9949*(theta2*180/Math.PI) + 863.13);
        //pwm = (int)(-9.9949*theta2);  // Given to us
        return pwm;
    }
